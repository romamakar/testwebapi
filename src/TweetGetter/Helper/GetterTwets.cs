...
namespace TweetGetter.Helper
{
    ...
    {
        public static async Task<List<Tweet>> GetProductAsync(HttpClient client, string path, DateTime startDate, DateTime endDate)
        {
            ...
            if (tweets.Count == 100)
            {
                tweets.AddRange(await GetProductAsync(client, path, Convert.ToDateTime(tweets[99].stamp), endDate));
            }
